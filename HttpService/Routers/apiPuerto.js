...
router.post('/savePuerto', function (req, res) {
    ...
    MongoClient.connect(url, function (err, db) {
        ...
        var collection = db.collection('puerto');
...
router.get('/getAllPuertoActivos', function (req, res) {
    var resultArray = [];

    MongoClient.connect(url, function (err, db) {
        assert.equal(null, err);

        var cursor = db.collection('puerto').find({ "estado": "1" });
...
router.get('/getAllPuerto', function (req, res) {
    ...
    MongoClient.connect(url, function (err, db) {
        ...
        assert.equal(null, err);
        var cursor = db.collection('puerto').find();
        cursor.forEach(function (doc, err) {
            assert.equal(null, err);
            resultArray.push(doc);
        }, function () {
